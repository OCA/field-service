    ...
    def action_open_change_logs(self):
        for location in self:
            ...
            if len(change_log_ids) > 1:
                ...
                vals['views'] = [
                    (self.env.ref('fieldservice_change_management.change_log_view_list').id,
                     'tree'),
                    (self.env.ref('fieldservice_change_management.change_log_view_form').id,
                     'form')]
            elif len(change_log_ids) == 1:
                vals['views'] = [(self.env.ref(
