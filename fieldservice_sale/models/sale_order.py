...
class SaleOrder(models.Model):
    ...
    def _field_create_fsm_order_prepare_values(self, line_id):
        self.ensure_one()
        ...
        return {
            ...
            'template_id': line_id.product_id.fsm_order_template_id.id
        }

    @api.multi
    def _field_create_fsm_order(self, line_id):
        ...
        for so in self:
            # create fsm_order
            values = so._field_create_fsm_order_prepare_values(line_id)
    ...
    @api.multi
    def _field_find_fsm_order(self, line_id):
        ...
        for so in self:
            ...
            if not fsm_order:
                fsm_order = so._field_create_fsm_order(line_id)[so.id]
            result[so.id] = fsm_order
        return result
