...
class TestFSMSaleOrder(TestFSMSale):
    ...
    def _isp_account_installed(self):
        ...
        if isp_account_module and \
           isp_account_module.state == 'installed':
            result = True
        return result
    ...
    def _fulfill_order(self, order):
        ...
        timesheet = self.env['account.analytic.line'].create({
            ...
            'product_id': self.env.ref(
                'fieldservice_isp_account.field_service_regular_time').id,
        })
        order.write({
    ...
    def test_sale_order_1(self):
        ...
        """
        sol = self.sol_service_per_order_1
        # Confirm the sale order
        self.sale_order_1.action_confirm()
        # 1 FSM order created
        self.assertEqual(
            len(self.sale_order_1.fsm_order_ids.ids), 1,
        ...
        self.assertEqual(
            len(fsm_order.ids), 1,
            'FSM Sale: Sale Order not linked to FSM Order')
        ...
        self.assertEqual(
            len(invoice.ids), 1,
            'FSM Sale: Sale Order 1 should create 1 invoice')
        self.assertTrue(
    ...
    def test_sale_order_2(self):
        ...
        # 1 order created
        self.assertEqual(
            len(self.sale_order_2.fsm_order_ids.ids), 1,
            'FSM Sale: Sale Order 2 should create 1 FSM Order')
        FSM_Order = self.env['fsm.order']
        fsm_order = FSM_Order.search(
            [('id', '=', sol.fsm_order_id.id)])
        ...
        self.assertEqual(
            len(invoice.ids), 1,
            'FSM Sale: Sale Order 2 should create 1 invoice')
        self.assertTrue(
            invoice.fsm_order_id.id == fsm_order.id,
            'FSM Sale: Invoice should be linked to FSM Order')
    ...
    def test_sale_order_3(self):
        ...
        self.assertEqual(
            len(self.sale_order_3.fsm_order_ids.ids), 2,
        ...
        self.assertEqual(
            len(invoice.ids), 2,
            'FSM Sale: Sale Order 3 should create 2 invoices')
        inv_fsm_orders = (x.fsm_order_id for x in invoice)
        self.assertTrue(
            fsm_order_1.id in inv_fsm_orders.ids,
            'FSM Sale: FSM Order should be linked to invoice')
        self.assertTrue(
            fsm_order_2.id in inv_fsm_orders.ids,
            'FSM Sale: FSM Order should be linked to invoice')
    ...
    def test_sale_order_4(self):
        ...
        # 3 orders created
        self.assertEqual(
            len(self.sale_order_4.fsm_order_ids.ids), 3,
        ...
        fsm_order_3 = FSM_Order.search(
            ...
            (['sale_line_id', '=', None]))
        self.assertTrue(
            len(fsm_order_3.ids), 1,
            'FSM Sale: FSM Order not linked to Sale Order')
        ...
        self.assertEqual(
            len(invoice.ids), 3,
            'FSM Sale: Sale Order 4 should create 3 invoices')
        inv_fsm_orders = (x.fsm_order_id for x in invoice)
        self.assertEqual(
