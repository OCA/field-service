...
class FSMRoutePayment(models.Model):
    ...
    amount_collected = fields.Float(
        string="Collected Amount", readonly=True, compute="_compute_amount_collected"
    )
    amount_counted = fields.Float(string="Counted Amount", default=0.0)
    difference = fields.Float(string="Difference", compute="_compute_amount_difference")
    ...
    def _compute_amount_difference(self):
        ...
        for dayroute_payment in self:
            dayroute_payment.difference = (
                dayroute_payment.amount_collected - dayroute_payment.amount_counted
            )
