...
class FSMRouteDayRoute(models.Model):
    ...
    invoice_count = fields.Integer(
        string="Invoice Count", compute="_compute_invoice_count", readonly=True
    )
    ...
    def write(self, values):
        ...
        for rec in self:
            if (
                ...
                and rec.stage_id.stage_type == "route"
                and rec.stage_id.is_closed
            ):
                for route_payment in rec.dayroute_payment_ids:
                    if route_payment.difference > 0:
                        ...
                        account_id = route_payment.journal_id.default_credit_account_id
                        lines = [
                            (
                                0,
                                0,
                        ...
                        route_payment.move_id = self.env["account.move"].create(
                            {
