...
class FSMOrder(models.Model):
    ...
    def _compute_schedule_time_range(self):
        ...
        raw_time_format = lang_obj.time_format or ""
        time_format = raw_time_format.replace(":%S", "").strip()
        if not time_format:
            time_format = raw_time_format.strip()
        if not time_format:
            time_format = "%H:%M"
        ...
        for order in self:
            ...
            start = fields.Datetime.context_timestamp(order, order.scheduled_date_start)
                ...
                fields.Datetime.context_timestamp(order, order.scheduled_date_end)
                if order.scheduled_date_end
                else start
            )

            start_date = start.strftime(date_format)
            start_time = start.strftime(time_format).strip()
            end_date = end.strftime(date_format)
            end_time = end.strftime(time_format).strip()
