...
from odoo.addons.fieldservice_sale.tests.test_fsm_sale_order import (
    TestFSMSale
)
...
class TestFSMSaleRecurring(TestFSMSale):
    ...
    def setUpFSMProducts(cls):
        ...
        cls.product_fsm_recur = cls.env['product.product'].create({
            ...
            'fsm_recurring_template_id': cls.env.ref(
                'fieldservice_recurring.recur_template_weekdays').id,
        })
    ...
    def test_fsm_sale_order_recurring(self):
        ...
        self.assertEqual(
            len(self.sale_order_recur.fsm_recurring_ids.ids), 1,
