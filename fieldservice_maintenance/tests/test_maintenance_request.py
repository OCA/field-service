...
class TestMaintenanceEquipment(TransactionCase):
    ...
    def test_maintenance_request_create(self):
        maintenance_equipment = self.env['maintenance.equipment'].\
            search([('name', '=', self.test_equipment.name),
                    ('is_fsm_equipment', '=', True)])
        maintenance_request = self.test_maintenance_request.\
            create({'name': 'Test Maintenance Request',
                    'maintenance_team_id': self.maintenance_team.id,
                    'equipment_id': maintenance_equipment.id})
        fsm_order = self.env['fsm.order'].\
            search([('type', '=', self.maintenance_order_type.id),
