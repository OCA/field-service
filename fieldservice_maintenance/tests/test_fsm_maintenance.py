...
class TestFSMMaintenance(TransactionCase):
    def test_fsm_maintenance(self):
        ...
        fsm_equip_01 = self.env["fsm.equipment"].create(
            {"name": "Test FSM Equipment 01", "current_location_id": fsm_loc.id}
        )
        ...
        maint_req_02 = self.env["maintenance.request"].create(
            {"name": "Equip 01 Request for FSM", "equipment_id": maint_equip_01.id}
        )
        ...
        with self.assertRaises(UserError) as e:
            self.env["maintenance.request"].create(
                {"name": "Equip 01 Request", "equipment_id": maint_equip_01.id}
            )
        self.assertEqual(
            e.exception.name,
