...
class FSMRecurringOrder(models.Model):
    ...
    fsm_order_by_year_count = fields.Integer(
        "Theoric Yearly order count", compute="_compute_tehoric_order_count",
    ...
    def _compute_tehoric_order_count(self):
        ...
        for recuring in self:
            rrules = recuring.fsm_frequency_set_id._get_rruleset(dtstart=date_start, until=date_end)
            recuring.fsm_order_by_year_count = len([date for date in rrules])
            recuring.fsm_order_by_month_count = recuring.fsm_order_by_year_count / 12.0 # number of month a year
