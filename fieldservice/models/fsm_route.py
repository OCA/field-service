...
class FSMRoute(models.Model):
    ...
    order_ids = fields.One2many("fsm.order", "route_id", string="Orders")
    person_id = fields.Many2one("fsm.person", string="Assigned To", required=True)
    ...
    def create(self, vals):
        if vals.get("name", _("New")) == _("New"):
            vals["name"] = self.env["ir.sequence"].next_by_code("fsm.route") or _("New")
